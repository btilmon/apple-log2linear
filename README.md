...
After decoding the log images to linear, the image intensity should increase linearly based on the exposure ratio. For example, if the exposure is increased from 1 second to 2 seconds, the average image intensity should increase 2x if only exposure varies. I captured a ProRes Log video with locked focus, iso, white balance, and manually decreased shutter speed during the video while looking at an evenly lit white wall. I used the blackmagic camera app for this on an iPhone 16 Pro Max. The GT radiance is computed by setting the first GT value to the first real average image intensity, and then multiplying it by the known exposure ratio for each exposure step. The real image intensities at each exposure are then compared to this GT to test if the decoded linear images are actually linear. The following plot suggests strong linearity of the decoded linear images.
